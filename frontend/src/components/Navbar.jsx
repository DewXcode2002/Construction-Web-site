    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-slate-900/90 backdrop-blur-md border-b border-slate-800 text-white transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center space-x-3 lg:space-x-5 xl:space-x-6 shrink-0">
            {navLinks.map((link) => (
              <button
                key={link.path}
                onClick={() => handleScrollLink(link.path)}
                className="text-slate-300 hover:text-amber-500 font-medium text-xs lg:text-sm transition-colors cursor-pointer whitespace-nowrap"
              >
                {link.name}
              </button>
            ))}
            ...
            <div className="flex items-center space-x-1 bg-slate-950/80 border border-slate-800 p-1 rounded-xl shrink-0">
            ...
            {user ? (
              <div className="flex items-center space-x-3 lg:space-x-4 border-l border-slate-800 pl-3 lg:pl-4 shrink-0">
                <Link
                  to={getDashboardLink()}
                  className="flex items-center space-x-2 text-xs lg:text-sm font-semibold bg-amber-500 text-slate-950 px-3 lg:px-4 py-2 rounded-lg hover:bg-amber-400 transition-colors whitespace-nowrap"
                >
                  <LayoutDashboard className="h-4 w-4" />
                  <span>{t.navDashboard}</span>
                </Link>
                <div className="flex items-center space-x-1.5 lg:space-x-2 text-slate-300 whitespace-nowrap">
              ...
              <Link
                to="/login"
                className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-semibold px-4 lg:px-5 py-2.5 rounded-lg text-xs lg:text-sm transition-all shadow-lg shadow-amber-500/10 cursor-pointer whitespace-nowrap"
              >
                {t.navLogin}
              </Link>
            )}
